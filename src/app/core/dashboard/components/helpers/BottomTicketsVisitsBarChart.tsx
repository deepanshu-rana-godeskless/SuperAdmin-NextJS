...
export function BottomTicketsVisitsBarChart({
  data,
  loading
}: {
  data?: any;
  ...
  const chartData = React.useMemo(() => {
    ...
    return [...data.results]
      .sort((a, b) => a.total_tickets_visits - b.total_tickets_visits)
      .slice(0, 5)
      .map((item) => ({
        tenant_name:
  ...
  const bottomTenant = chartData.length > 0 ? chartData[0] : null;
    ...
    <Card className='@container/card w-full max-w-[520px] gap-2 overflow-hidden p-3'>
      <CardHeader className='space-y-0.5 px-0 pb-1'>
        <CardTitle className='text-sm font-semibold'>
          Bottom 5 Tickets & Visits
        </CardTitle>
        <CardDescription className='text-xs'>
          Tenants with the lowest combined tickets and visits
        </CardDescription>
      </CardHeader>
          ...
          <div className='flex w-full flex-col gap-2'>
            ...
            <div className='flex w-full items-center justify-between'>
              <span className='max-w-[200px] truncate font-medium'>
                {bottomTenant.tenant_name}
              </span>
              <span className='font-semibold tabular-nums'>
                {bottomTenant.total_tickets_visits}
